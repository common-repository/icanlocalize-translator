...
function ICanLocalizeValidate($args){ 
        ...
        $current_settings = get_option('iclt_settings');
        if(empty($current_settings['site_id']) && empty($current_settings['access_key'])){
            ...
            update_option('iclt_settings', $current_settings);        
        }
...
function ICanLocalizeGetPagesOrder($args){
    global $iclt, $wpdb;
    ...
    $settings = $iclt->get_settings();
    if($settings['access_key'] != $access_key){
        return array('err_code'=>1, 'err_str'=>sprintf( __( 'ERROR - Invalid access hey')));
    }
    $pages = $wpdb->get_results("SELECT ID, menu_order FROM {$wpdb->posts} WHERE post_type='page' ORDER BY menu_order, ID");
    foreach($pages as $p){
        $pages_ordered[] = $p->ID;
    }
    return array('err_code'=>0, 'pages_ordered'=>$pages_ordered);
